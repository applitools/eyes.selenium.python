...
class TestDomCaptureUnit(object):
    ...
    @pytest.mark.test_page_url('https://applitools-dom-capture-origin-1.surge.sh/test.html')
    def test_send_dom_simple_HTML(self):
        actual_dom_json = dom_capture.get_full_window_dom(self.driver, return_as_dict=True)
        ...
        assert sorted(actual_dom_json['rect'].keys()) == sorted(expected_dom_json['rect'].keys())
        assert actual_dom_json['style'] == expected_dom_json['style']
        assert len(actual_dom_json['childNodes']) == len(expected_dom_json['childNodes'])
        assert actual_dom_json['childNodes'][0]['tagName'] == expected_dom_json['childNodes'][0]['tagName']
        assert actual_dom_json['childNodes'][-1]['tagName'] == expected_dom_json['childNodes'][-1]['tagName']

    @pytest.mark.test_page_url('https://booking.kayak.com/flights/TLV-MIA/2018-11-15/2019-04-31?sort=bestflight_a')
    ...
    @pytest.mark.test_page_url('http://applitools.github.io/demo/TestPages/DomTest/dom_capture.html')
    ...
    @pytest.mark.test_page_url('http://applitools.github.io/demo/TestPages/DomTest/dom_capture_2.html')
    ...
    @pytest.mark.test_page_url('https://booking.kayak.com/flights/TLV-MIA/2018-09-25/2018-10-31?sort=bestflight_a')
    ...
    @pytest.mark.test_page_url('https://nikita-andreev.github.io/applitools/dom_capture.html?aaa')
    def test_send_dom_nsa(self):
        expected_json = self._get_expected_json('test_send_dom_nsa')
        dom_json = dom_capture.get_full_window_dom(self.driver, return_as_dict=True)
        assert dom_json['css'] == expected_json['css']
        inner_css = lambda o: o['childNodes'][1]['childNodes'][3]['childNodes'][0]['css']
...
class TestCustomersPages(object):
    ...
    @pytest.mark.test_page_url(
        'https://www.amazon.com/stream/cd7be774-51ef-4dfe-8e97-1fdec7357113/ref=strm_theme_kitchen?asCursor=WyIxLjgiLCJ0czEiLCIxNTM1NTQyMjAwMDAwIiwiIiwiUzAwMTc6MDpudWxsIiwiUzAwMTc6MjoxIiwiUzAwMTc6MDotMSIsIiIsIiIsIjAiLCJzdWI0IiwiMTUzNTU5NDQwMDAwMCIsImhmMS1zYXZlcyIsIjE1MzU2MDE2MDAwMDAiLCJ2MSIsIjE1MzU2MDUyMDQyMzgiLCIiLCIwIiwidjEiLCIxNTM1NTg1NDAwMDAwIl0%3D&asCacheIndex=0&asYOffset=-321&asMod=1')
    ...
    @pytest.mark.test_page_url('https://www.games-workshop.com/en-US/The-Beast-Arises-Omnibus-1-2018')
    @pytest.mark.eyes(send_dom=True)
